...
- `-experimental` — route compilation through the new AST→IR pipeline (see Architecture) instead of the legacy direct-from-AST codegen. Functions the new backend cannot emit yet fall back to the legacy one, so this produces a working executable for most inputs rather than only for the subset it covers.
...
- `-irDump <file>` — dump the IR (experimental pipeline); also emits `cfg.dot` for the CFG. Optionally `-irDump:phase[,phase...] <file>` (phases: `initial`, `ssa`, `scp`, `gvn`, `dce`, `mir`, `isel`, `ra`) snapshots the IR immediately after each named pass in `translateFunction()` (`src/ir/ast2ir.c`) instead of only the fully-processed result — e.g. `-irDump:ssa` captures IR right after `buildSSA`, unaffected by whatever `scp`/`gvn`/`dce` do afterwards. The last three are the odd ones out: they dump the `MachineFunction` rather than the IR, in machine-IR form — `mir` as stage 0 leaves it (CFG, frame, phi copies), `isel` once instruction selection has filled the blocks in, `ra` once register allocation has replaced every virtual register.
- `-noFallback` / `-allowFallback <fn>` — only with `-experimental`. `-noFallback` makes it an error (nonzero exit) when a function the IR backend can't emit is handed to the legacy one; `-allowFallback` (repeatable) exempts one function by name, and a name that is listed but did *not* fall back is an error too, so an exemption can't outlive what it excuses.
...
# The same fixtures compiled through the IR pipeline instead
python3 test/testRunner.py -c build/bin/main -wd /tmp/eduwd -p test/testData/codegen -m codegen --compiler-flag=-experimental
```
...
- `--no-fallback` (codegen mode) passes `-noFallback` to every compilation, so a fixture whose functions get handed back to the legacy backend fails instead of silently passing on legacy output. Exemptions live in a `<name>.fallback` sibling: one function name per line, `#` starts a comment, no file means no function may fall back. The `codegen_experimental` CTest entry runs with this on.
...
- A `<name>.experimental` sibling (reason as its contents) marks a fixture that only the `-experimental` backend compiles correctly and that the legacy one is not going to be taught to — a VLA in a loop, say, whose storage the legacy backend never gives back. Such a test is **skipped** in a run without `-experimental` and listed under `Skipped (need -experimental)`; it is not muted, because muting is for a bug someone intends to fix and a muted test is flagged the day it starts passing.
...
4. After a clean parse, compilation forks into **two independent backend pipelines** selected by `-experimental`:
   ...
   - **Experimental IR pipeline** (`-experimental`): `translateAstToIr()` (`src/ir/ast2ir.c`) lowers AST to a CFG-based SSA-capable IR (`include/ir/ir.h`, `include/ir/instructionList.h`). Passes: `buildSSA` (`src/ir/ssa.c`), `buildDominatorInfo` (`src/ir/dominators.c`), `gvn` — global value numbering (`src/ir/gvn.c`), `scp`/`cp` — (sparse) constant propagation (`src/ir/cp.c`, `src/ir/evaluator.c`), `dce` — dead code elimination (`src/ir/dce.c`). `IrFunction.phases` bitflags (`initalIr`/`ssa`/`cp_1`/`gvn`) track which passes a function has been through. The optimized IR then goes through a four-stage machine backend under `src/ir/codegen/`: `prepareMachineFunction` (stage 0 — critical edges, phi destruction, frame layout, `src/ir/codegen/prepare.c`), `selectInstructions` (stage 1, `isel.c` + `src/x86_64/isel_x86_64.c`), `allocateRegisters` (stage 2, `regalloc.c` — currently a spill-everything allocator) and emission (stage 3, `src/x86_64/emit_x86_64.c`), producing the same `GeneratedFunction` the legacy backend does.

     This backend does not cover the whole language yet, so **the choice between the two is per function, not per file**: `generateCodeForFile()` uses the IR backend for any function `canEmitMachineFunction()` accepts (nothing left unselected, and register allocation did not decline it) and falls back to `generateFunction` for the rest. A single object file routinely contains functions from both; they meet through the ordinary SysV ABI. `docs/ir-codegen-design.md` is the design document and tracks what is done and what is not.

...
- `-experimental` — route compilation through the new AST→IR pipeline (see Architecture) instead of the legacy direct-from-AST codegen. Every function goes through it: there is no per-function fallback any more, so a construct the IR backend cannot build aborts the compiler rather than being handed to the legacy one. x86_64 only — `-experimental` with any other `-march` is rejected.

...
Nothing here is an oracle EduCC's own suite already has: `crossabi` compares EduCC to EduCC, so a classifier both backends share is invisible to it, where tinycc's `abitest` compares EduCC to a third party. It has found four bugs nothing else did — an x87 conversion that stored its operand before defining it, SysV's aggregate classification, and two front-end bugs that give a file-scope object more than one address, which only a single-translation-unit build (`memtest`, `ONE_SOURCE=1`) can expose.

As of the last full run the three IR configurations pass 14 of tinycc's 15 test sections and `-legacy` 13, against 15 for gcc at every level and for tcc itself. The one every EduCC column fails is `dlltest`, which needs `-fPIC`; the extra one `-legacy` fails is `abitest`, which is the deliberate aggregate-ABI disagreement above.

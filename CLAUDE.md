...
Since roadmap step 30 a bootstrap goes through the **IR backend**, and since step 37 through its graph-colouring register allocator — the script passes no flag either time and did not have to change. The compiler it produces is now 1.29 MB, against 1.45 MB from `-Xregalloc=linear` and 1.34 MB from `-legacy`, the first time the self-compiled binary has been the smallest of the three. Given the same workload it runs 17% faster than the `-legacy`-built one and 6% faster than the linear-scan-built one, where before step 34 it was 1.6× *slower* than legacy. Five stages still take under 40 seconds. `-DCMAKE_C_FLAGS=-legacy` or `-DCMAKE_C_FLAGS=-Xregalloc=linear` bootstraps one of the others.

  ...
  if (!hasError) {
    if (config->experimental) {
      ...
      if (!config->skipCodegen) {
        ...
        // The AST still has to be canonicalized, because every function the IR
        // backend cannot emit yet is generated by the legacy one from this
        // same tree - and that one requires it. It happens after translation
        // rather than before so that the IR is built from the tree the
        // frontend produced, not from a lowered one.
        cannonizeAstFile(&context, astFile);

        generateCodeForFile(&context, &cg, astFile, &irFunctions);
      }
      ...
      if (!config->skipCodegen) {
        ...
        GeneratedFile *genFile = generateCodeForFile(&context, &cg, astFile, NULL);
      }
    }
  }
